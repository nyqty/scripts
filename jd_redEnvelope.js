...
rebateCodes = $.isNode() ? (process.env.redEnvelope_rebateCode ? process.env.redEnvelope_rebateCode : `${rebateCodes}`) : ($.getdata('redEnvelope_rebateCode') ? $.getdata('redEnvelope_rebateCode') : `${rebateCodes}`);
...
function getUA(){
  ...
  rebateCode = 'SduCfLF'
  if($.index != 1){
    let arr = [rebateCodes,'SduCfLF']
    rebateCode = arr[Math.floor(Math.random() * arr.length)] || rebateCode
    if(!rebateCode) rebateCode = 'SduCfLF'
  }
