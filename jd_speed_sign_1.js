...
活动时间：长期
活动入口：京喜特价app-现金签到
cron "2 0-23/8 * * *" script-path=jd_speed_sign_1.js,tag=京喜特价-1
*/
...
!(async () => {
	...
	const date = new Date()
	$.last_day = new Date(date.getFullYear(), date.getMonth() + 1, 0)
		.getDate() == date.getDate()
	let max = cookiesArr.length > 70 ? 70 : cookiesArr.length;
	for (let i = 0; i < max; i++) {
		if (cookiesArr[i]) {
			cookie = cookiesArr[i];
