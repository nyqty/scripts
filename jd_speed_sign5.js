...
活动时间：长期
活动入口：京东极速版app-现金签到
cron "8 0 0-23/8 * * *" script-path=jd_speed_sign5.js,tag=京东极速版-5
*/

const $ = new Env('京东极速版-5');
...
!(async () => {
  ...
  let max=cookiesArr.length;//cookiesArr.length>100?100:cookiesArr.length;
  for (let i = 80; i < max; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
