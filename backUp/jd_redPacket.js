...
[task_local]
#京东全民开红包
5 0,12,20 * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_redPacket.js, tag=京东全民开红包, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_redPacket.png, enabled=true
===================Loon==============
[Script]
cron "5 0,12,20 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_redPacket.js, tag=京东全民开红包
===============Surge===============
[Script]
京东全民开红包 = type=cron,cronexp="5 0,12,20 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_redPacket.js
====================================小火箭=============================
京东全民开红包 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_redPacket.js, cronexpr="5 0,12,20 * * *", timeout=3600, enable=true
 */
...
!(async () => {
  ...
  /*
  let res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/atyvcn/updateTeam@master/shareCodes/jd/red.json')
  if (!res) {
...
function taskHomePage() {
  return new Promise((resolve) => {
    $.post(taskUrl(arguments.callee.name.toString(), {"clientInfo":{},"random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"}), (err, resp, data) => {
      try {
        if (err) {
          console.log(`\n${$.name}: API查询请求失败 ‼️‼️`);
          console.log(JSON.stringify(err));
...
function startTask(taskType) {
  ...
  let data = {taskType,"random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"};
...
function getTaskDetailForColor(taskType) {
  const data = {"clientInfo":{}, taskType,"random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"};
  return new Promise((resolve) => {
    $.post(taskUrl(arguments.callee.name.toString(), data), (err, resp, data) => {
      try {
        if (err) {
...
function taskReportForColor(taskType, detailId) {
  const data = {taskType, detailId,"random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"};
...
function receiveTaskRedpacket(taskType) {
  const body = {"clientInfo":{}, taskType,"random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"};
  return new Promise((resolve) => {
...
function jinli_h5assist(redPacketId) {
  ...
  const body = {"clientInfo":{},redPacketId,"followShop":0,"promUserState":"","random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"};
...
function h5receiveRedpacketAll() {
  const options = taskUrl(arguments.callee.name.toString(), {"clientInfo":{},"random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"})
  return new Promise((resolve) => {
    $.post(options, (err, resp, data) => {
      try {
        if (err) {
...
//发起助力红包API
function h5launch() {
  const body = {"followShop":1,"random":random(000000, 999999),"log":"4817e3a2~8,~1wsv3ig","sceneid":"JLHBhPageh5"};
  const options = taskUrl(arguments.callee.name.toString(), body)
  return new Promise((resolve) => {
    $.post(options, (err, resp, data) => {
      try {
...
function h5activityIndex() {
  const body = {"clientInfo":{},"isjdapp":1,"random":random(00000000, 99999999),"log":`${new Date()}~1aTmT9wMbemMDF3c0xyVzAxMQ==.`,"sceneid":"JLHBhPageh5"};
  const options = taskUrl(arguments.callee.name.toString(), body);
  return new Promise((resolve) => {
    $.post(options, async (err, resp, data) => {
      try {
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            console.log('GetJDUserInfoUnion 京东服务器返回空数据');
          }
        }
      } catch (e) {
        $.logErr(e)
