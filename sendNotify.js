/**
 * @Last Modified time: 2021-10-14 15:00:54
 * sendNotify 推送通知功能
 * @param text 通知头
 * @param desp 通知体
...
let GO_CQHTTP_TOKEN = ''//填写在go-cqhttp文件设置的访问密钥 access-token
...
if (process.env.GO_CQHTTP_TOKEN) {
  GO_CQHTTP_TOKEN = process.env.GO_CQHTTP_TOKEN;
}
...
function GetUser() {
  try {
    fs.accessSync('../db/env.db')
    let env = fs.readFileSync('../db/env.db').toString().split("\n");//console.log( env.length );
    let json = {};
    return env.reduce(function (init, item, i) {
      try {
        if (json = JSON.parse(item)) if (json.value) {
          init.push({
            uname: decodeURIComponent(json.value.match(/pt_pin=([^; ]+)(?=;?)/) && json.value.match(/pt_pin=([^; ]+)(?=;?)/)[1]),
            qq: json.remarks.match(/qq=([^; ]+)(?=;?)/) && json.remarks.match(/qq=([^; ]+)(?=;?)/)[1],
          })
        }
      } catch (e) {
        //console.log(`\nx错误${i}`+item);
...
function GetUserQQ(uname) {
  try {
    fs.accessSync('../db/env.db')
    let env = fs.readFileSync('../db/env.db').toString().split("\n");//console.log( env.length );
    for (let i = 0, json, arr, pin; i < env.length; i++) {
      try {
        if (json = JSON.parse(item)) if (json.value) {
          arr = json.value.match(/pt_pin=([^; ]+)(?=;?)/);
          pin = arr ? arr[1] : "";
          if (pin && pin == uname) {
            arr = json.remarks.match(/qq=([^; ]+)(?=;?)/);
            if (arr) return arr[1];
          }
        }
      } catch (e) {
...
async function sendNotify(text, desp, params = {}, author) {
  ...
  let uname="",title = text.match(/.*?(?=\s?-)/g) ? text.match(/.*?(?=\s?-)/g)[0] : text;
  //东东农场 - 账号14 - NoKYbCwcSoRO水果已可领取
  let expire = RegExp("cookie已失效");// desp 重新登录
  let individual = RegExp("账号\\d+|已可领取|忘了");//东东农场 - 您忘了种植新的水果
  if ( expire.test(text) === true || individual.test(text) === true ) {
    uname = GetUserName();
  }
  await Promise.all([
    TYNotify(uname, uname?title:text, desp)//text title
  ]);
  ...
  if (tg_only) {
    ...
    await Promise.all([
      BarkNotify(title, desp, params), //iOS Bark APP
      tgBotNotify(title, desp), //telegram 机器人
      ddBotNotify(title, desp), //钉钉机器人
      qywxBotNotify(title, desp), //企业微信机器人
      qywxamNotify(title, desp), //企业微信应用消息推送
      iGotNotify(title, desp, params), //iGot
      //CoolPush(title, desp)//QQ酷推
      goCQhttp(uname, title, desp)  // go-cqhttp
    ])
  }
}
...
function TYNotify(uname, title, content) {
  if (TYGOBOT_URL) {
    return new Promise((resolve) => {
      const options = {
        url: TYGOBOT_URL,
        body: `uname=${uname}&title=${title}&content=${content}`,
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
        timeout,
...
function goCQhttp(uname, title, desp) {
  if (GO_CQHTTP_URL && GO_CQHTTP_QQ && GO_CQHTTP_METHOD) {
    let message = (title + '\n' + desp);
    let recv_id = '', METHOD = GO_CQHTTP_METHOD;
    ...
    let qqnumber = "";
    if (uname){
      qqnumber = GetUserQQ(uname)
      if( qqnumber ){
        message = GO_CQHTTP_LINK ? (message + '\n\n' + GO_CQHTTP_LINK) : message
        METHOD = "send_private_msg"
        recv_id = "user_id"
      }
    }
    if( !qqnumber ) qqnumber=GO_CQHTTP_QQ
    return new Promise(resolve => {
      const options = {
        url: `http://${GO_CQHTTP_URL}/${METHOD}?access_token=${GO_CQHTTP_TOKEN}&${recv_id}=${qqnumber}`,
        ...
        try {
          if (err) {
            ...
            if (data.retcode === 0 && data.status === 'ok') {
              console.log(`go-cqhttp发送给通知消息成功🎉\n`)
            } else if (data.retcode !== 0 && data.status !== 'ok') {
              console.log(`go-cqhttp发送通知消息异常\n${JSON.stringify(data)}`)
              console.log(`http://${GO_CQHTTP_URL}/${METHOD}?access_token=${GO_CQHTTP_TOKEN}&${recv_id}=${qqnumber}`)
            }
          }
        } catch (e) {
          $.logErr(e, resp);
