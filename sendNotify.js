 ...
 */
 ...
 if (process.env.TYGOBOT_URL) {
   TYGOBOT_URL = process.env.TYGOBOT_URL;
 }
     ...
     return env.reduce(function (init, item, i) {
       try {
         if (json = JSON.parse(item)) if (json.value) {
           init.push({
             name: decodeURIComponent(json.value.match(/pt_pin=([^; ]+)(?=;?)/) && json.value.match(/pt_pin=([^; ]+)(?=;?)/)[1]),
 ...
 async function sendNotify(text, desp, params = {}, author="") {
   ...
   let name="",title = text.match(/.*?(?=\s?-)/g) ? text.match(/.*?(?=\s?-)/g)[0] : text;
 ...
 function TYNotify(name, title, content) {
   if (TYGOBOT_URL) {
     return new Promise((resolve) => {
       const options = {
         url: TYGOBOT_URL,
         body: `name=${name}&title=${title}&content=${content}`,
         headers: {
           'Content-Type': 'application/x-www-form-urlencoded',
         },
         timeout,
         ...
         try {
           if (err) {
             ...
             if (data.code === 200) {
               ...
               console.log(`TY发送通知消息异常${TYGOBOT_URL}\n${data.msg}`);
             }
           }
         } catch (e) {
           $.logErr(e, resp);
 ...
 function goCQhttp(name, title, desp) {
