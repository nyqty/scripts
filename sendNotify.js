 ...
 * @Last Modified by: whyour,TY
 * @Last Modified time: 2022-7-27 12:00:00
 * sendNotify 推送通知功能
 * @param text 通知头
 * @param desp 通知体
...
let BARK_ICON = 'http://q1.qlogo.cn/g?b=qq&nk=1659670408&s=100';
 ...
 * @param desp 通知体
 * @param params 某些推送通知方式点击弹窗可跳转, 例：{ url: 'https://abc.com' }
 * @param author 作者仓库等信息  例：`本脚本免费使用 By：TY`
          ...
          try {
            if (err) {
              ...
              if (data.retcode === 0) {
                ...
                console.log(
                  `go-cqhttp发送通知消息异常\n${JSON.stringify(data)}`,
                  console.log(`${GOBOT_URL}?access_token=${GOBOT_TOKEN}&${GOBOT_QQ}`)
                );
              }
            }
          } catch (e) {
