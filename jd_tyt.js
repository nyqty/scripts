...
注意：助力码每天会变，旧的不可用。
助力逻辑：优先助力互助码变量，北京时间15点后默认会助力【atyvcn】，介意请勿运行！
*/
const $ = new Env('柠檬推一推');
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      await $.wait( getRndInteger(1000, 3000) )
      if ($.canRun) {
        await getCoinDozerInfo()
        await $.wait( getRndInteger(500, 2000) )
        await coinDozerBackFlow()
        await $.wait( getRndInteger(500, 2000) )
        await helpCoinDozer(packetId)
        await $.wait( getRndInteger(200, 600) )
        for (let j = 0; j < tytpacketId_Array.length; j++) {
          let tytpacketId = tytpacketId_Array[j];
          console.log(`\n去助力助力码:${tytpacketId}`)
          let code = await tythelp(tytpacketId)
          if ( code == 100 || code == 703) {//参数错误 已完成砍价
            tytpacketId_Array.splice(j, 1)
            j--
            continue  
            ...
            await $.wait( getRndInteger(10000, 20000) )
          }
          await $.wait( getRndInteger(1000, 3000) )
        }
      }
...
//h5st: 20211031212022682;1366567041939163;10005;tk01w76221abd30nlqQ8UaHhb1+aaCR6KS2rSQEg4QqFVSZJBtWT45BXw4qrb5oGT9oaHvA3ZP5HLn1VgRNwAKN+EFTj;39a0a50d920a3141ffb1dc3e2151ab9de8a65df084718df07aed5df71a81613f
...
function tythelp(tytpacketId) {
  return new Promise(async (resolve) => {
    let options = {
      url: `https://api.m.jd.com/?t=${Math.round(new Date())}`,
      body: `functionId=helpCoinDozer&appid=station-soa-h5&client=H5&clientVersion=1.0.0&t=${Math.round(new Date())}&body={"actId":"d5a8c7198ee54de093d2adb04089d3ec","channel":"coin_dozer","antiToken":"","referer":"-1","frontendInitStatus":"s","packetId":"${tytpacketId}","helperStatus":"0"}&_ste=1&_stk=appid,body,client,clientVersion,functionId,t&h5st=20210608104303790;8489907903583162;10005;tk01w89681aa9a8nZDdIanIyWnVuWFLK4gnqY+05WKcPY3NWU2dcfa73B7PBM7ufJEN0U+4MyHW5N2mT/RNMq72ycJxH;7e6b956f1a8a71b269a0038bbb4abd24bcfb834a88910818cf1bdfc55b7b96e5`,
      headers: {
        "Origin": "https://pushgold.jd.com",
        "Host": "api.m.jd.com",
        "User-Agent": "jdltapp;iPhone;3.3.6;14.3;75aeceef3046d8ce11d354ff89af9517a2e4aa18;network/wifi;hasUPPay/0;pushNoticeIsOpen/0;lang/zh_CN;model/iPhone9,2;addressid/4585826605;hasOCPay/0;appBuild/1060;supportBestPay/0;pv/53.31;apprpd/;ref/https://invite-reward.jd.com/?lng=106.286950&lat=29.969353&sid=547255867e847394aedfb6d68c3e50fw&un_area=4_48201_54794_0#/invitee?inviterId=dS%2Bp85VyjydPuAOOnFP%2Faw%3D%3D;psq/0;ads/;psn/75aeceef3046d8ce11d354ff89af9517a2e4aa18|89;jdv/0|kong|t_1001003207_1762319_6901310|jingfen|30578707801140d09fcd54e5cd83bbf7|1621510932517|1621511027;adk/;app_device/IOS;pap/JA2020_3112531|3.3.6|IOS 14.3;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1",
...
function getAuthorShareCode() {
  return new Promise(resolve => {
      $.get({
          url: "https://raw.fastgit.org/atyvcn/updateTeam/master/shareCodes/jd/tyt.json",
          headers: {
          ...
          try {
              if (err) {
                  ...
                  if ($.authorCode && $.authorCode.length) {
                    console.log(`\n******会助力【atyvcn】，介意请勿运行******\n`);
                    tytpacketId_Array = [...new Set([...tytpacketId_Array, ...$.authorCode])];
                  }
              }
          } catch (e) {
              $.logErr(e, resp)
...
async function taskPostUrl(functionId, body) {
  return {
    url: `${JD_API_HOST}`,
    body: `functionId=${functionId}&body=${escape(JSON.stringify(body))}&client=wh5&clientVersion=1.0.0&appid=content_ecology&uuid=6898c30638c55142969304c8e2167997fa59eb54&t=${Math.round(new Date())}`,
    headers: {
      'Cookie': cookie,
      'Host': 'api.m.jd.com',
