...
10 0 * * * https://raw.githubusercontent.com/cdle/jd_study/main/jd_angryKoi.js
*/
const $ = new Env("愤怒的锦鲤")
...
!(async () => {
    ...
    for (let i in cookiesArr) {
        // 将用于助力的账号加入列表
        tools.push({UserName: decodeURIComponent(cookiesArr[i].match(/pt_pin=([^; ]+)(?=;?)/) && cookiesArr[i].match(/pt_pin=([^; ]+)(?=;?)/)[1]), assisted: false, cookie: cookie})
    }
        ...
        try {
            ...
            if (help) {
                while (tools.length > 0 && remainingTryCount > 0) {
                    ...
                    let tool = tools.pop()
                    $.UserName = decodeURIComponent(tool.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && tool.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
                    // 特殊处理自己的账号
                    ...
                    console.debug(`尝试用 ${tool.UserName} 账号助力 ${help.UserName} 账号，用于互助的账号剩余 ${tools.length}`)
                    ...
                    if (help.assist_full) {
                        console.info(`账号 ${help.UserName} 助力完成，累计获得 ${help.helpCount} 次互助，将尝试下一个账号`)
                        break
                    }
...
async function helpThisUser(help, tool) {
    ...
    await requestApi('jinli_h5assist', tool.cookie, {
        ...
        console.log(`${tool.UserName}->${help.UserName}`, desc)
    })
}
