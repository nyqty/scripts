            ...
            try {
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        $.gameToken = data.token//$.token3
                        $.strength = data.role.items['8003']
                        $.datalist = {
                            "gameId": $.gameId,
                            "token": $.gameToken,
                            "enc": $.gameToken.slice(8, 16) + $.gameToken.slice(24, 32),
                ...
                try {
                    if (err) {
                        ...
                        if (safeGet(data)) {
                            data = JSON.parse(data)
                            if(data.__data__) {
                                let i = body.__id__ == $.datalist.gameId ? 4 : 3,
                                j = GetData(data, data.__iv__, i);
                                console.log(data)
                                data = JSON.parse(j);
                                if (data.code === 0) {
...
function GetData(data, iv, i) {
    (i == 1 || i == 2) && typeof data == "object" && (data = JSON.stringify(data));
    switch (i) {
        case 1:
    ...
    if (!data.__data__) {
        ...
        var l = CryptoJS.AES.decrypt(data.__data__, CryptoJS.enc.Utf8.parse(key), {
            "iv": CryptoJS.enc.Utf8.parse(iv),
            "mode": CryptoJS.mode.CBC,
            "padding": CryptoJS.pad.Pkcs7
        }),
