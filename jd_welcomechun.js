    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if(res && res.data && res.code==0){
            ...
            if(bizCode==0){
                ...
                console.log(`当前红包${result.awardInfo?.hongbao}，击鼓${$.coupon}次`);
                //result.lotteryFillTimes
                ...
                if(result.needSendWelcomeHongbao){
                    ...
                    res=await taskPost("party_welcome", {"area":"0_0_0_0","uuid":$.UUID},"h5st=");
                    if(res && res.data && res.code==0){
                        let {bizCode,bizMsg,result}=res.data
                        if(bizCode==0){
...
async function party_lottery() {
    console.log(`开始击鼓~`);
    let res=await taskPost("party_lottery", {"area":"0_0_0_0","uuid":$.UUID},"h5st=");
    if(res && res.data && res.code==0){
        let {bizCode,bizMsg,result}=res.data
        if(bizCode==0){
            $.coupon=result.awardInfo?.coupon;
            console.log(`${result.title}当前红包${result.awardInfo?.hongbao}，击鼓${$.coupon}次`)
            //console.log(`当前回合奖励`+msg.join("\n"));
            currentRoundAward(result.currentRoundAward?result.currentRoundAward:result.award);
        }else if(bizCode==-302){
            console.log(bizMsg);
...
function taskPost(functionId, body,get='') {
...
function getUA() {
    ...
    /*jdapp;android;11.4.2;;;appBuild/98638;ef/1;ep/{"hdid":"JM9F1ywUPwflvMIpYPok0tt5k9kW4ArJEU3lfLhxBqw=","ts":1673516247578,"ridx":-1,"cipher":{"sv":"CJS=","ad":"CJunCJvvZJqnZQOnZNGyYq==","od":"DNS5YwG5DQSnD2YyEQHuDG==","ov":"CzO=","ud":"CJunCJvvZJqnZQOnZNGyYq=="},"ciphertype":5,"version":"1.2.0","appname":"com.jingdong.app.mall"};jdSupportDarkMode/0;Mozilla/5.0 (Linux; Android 12; 22081212C Build/SKQ1.220303.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/89.0.4389.72 MQQBrowser/6.2 TBS/046141 Mobile Safari/537.36*/
